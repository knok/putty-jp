		      ...
		      unsigned long attr, int lattr)
{
	...
	/* > transparent background patch */
		    exttextout_options, &line_box, uni_buf, nlen, IpDx_maybe, !!(attr & ATTR_WIDE));
	/* < */
			...
			ETO_CLIPPED, &line_box, uni_buf, nlen, IpDx_maybe, !!(attr & ATTR_WIDE));
            }
            ...
            ExtTextOut(hdc, x + xoffset,
                       y - font_height * (lattr == LATTR_BOT) + text_adjust,
	/* > transparent background patch */
		   exttextout_options, &line_box, directbuf, len, IpDx_maybe);
	/* < */
            if (bold_mode == BOLD_SHADOW && (attr & ATTR_BOLD)) {
                SetBkMode(hdc, TRANSPARENT);
            ...
            if (bold_mode == BOLD_SHADOW && (attr & ATTR_BOLD)) {
                ...
                ExtTextOutW2(hdc, x + xoffset - 1,
                            y - font_height * (lattr ==
                                               LATTR_BOT) + text_adjust,
			ETO_CLIPPED, &line_box, wbuf, len, IpDx_maybe, !!(attr & ATTR_WIDE));
            }
        }
